...
- (id)initWithFrame:(CGRect)frame imageCacheManager:(CMImageCacheManager *)imageCacheManager;
{
    ...
    if (self)
    {
        ...
        [_contentLabel applyStyle:[AGJiPinStyle WeiboRefContentStyle]];
        _contentLabel.numberOfLines = 0;
        [self addSubview:_contentLabel];
        [_contentLabel release];

        _commentButton = [[AGWeiboButton alloc] initWithImageNamed:@"DetailContentCommentIcon.png" style:[AGJiPinStyle WeiboButtonNoBorderSmallTextStyle] withBorder:NO];
        _replyButton = [[AGWeiboButton alloc] initWithImageNamed:@"DetailContentReplayIcon.png" style:[AGJiPinStyle WeiboButtonNoBorderSmallTextStyle] withBorder:NO];
...
- (void)setStatus:(SSSinaWeiboStatusInfoReader *)status
{
    ...
    _commentButton .imageEdgeInsets = UIEdgeInsetsMake(0, 0.0, 0, 3.0);
    _commentButton.titleEdgeInsets = UIEdgeInsetsMake(3.0, 3.0, 0, 3.0);
    _commentButton.frame = CGRectMake(self.width - _commentButton.width - 5.0, height + VERTICAL_GAP, _commentButton.width, _commentButton.height);
    
    [_replyButton setTitle:[NSString stringWithFormat:@"%d", status.repostsCount]
                  forState:UIControlStateNormal];
    _replyButton .imageEdgeInsets = UIEdgeInsetsMake(0, 0.0, 0, 3.0);
    _replyButton.titleEdgeInsets = UIEdgeInsetsMake(3.0, 3.0, 0, 3.0);
    _replyButton.frame = CGRectMake(_commentButton.left - _replyButton.width - 3.0, height + VERTICAL_GAP, _replyButton.width, _replyButton.height);
    
    height = _replyButton.bottom;
